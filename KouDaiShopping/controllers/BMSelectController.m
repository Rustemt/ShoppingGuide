...
@interface BMSelectController ()<UITableViewDelegate,UITableViewDataSource,JDRequestDelegate>
{
    JDClient   *jdClient;
...
@implementation BMSelectController
...
- (void)viewDidLoad
{
    ...
    //页码
    [jdClient requestWithMethodName:@"jingdong.ware.stroll.special.list.get" andParams:paramsDic andHttpMethod:@"POST" andDelegate:self];
    if ([[BMCommon sharedBMCommon].type isEqualToString:@"优惠精选"]) {
        self.title = @"优惠精选";
    }else
    {
        self.title = @"特卖精选";
...
-(void)parseResponseResult:(NSDictionary *)response{
    ...
    }else if ([requestMethod isEqualToString:@"jingdong_ware_stroll_special_list_get_responce"]) {
        NSMutableDictionary *wareDic = [responseDic objectForKey:@"specialPriceWareList"];
//        NSArray *array = wareDic[@"specialPriceWareList"];
        resultArray = [wareDic objectForKey:@"specialPriceWare"];
    }
    ...
    if (resultArray && [resultArray count] > 0) {
        ...
        for (NSDictionary *obj in resultArray) {
            BMSelectModle *model = [BMSelectModle initWithDict:obj];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[BMCommon sharedBMCommon].type isEqualToString:@"特卖精选"]) {
        model = m_dataSource[random() % m_dataSource.count];
    }
