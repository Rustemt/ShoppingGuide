...
-(id)initWithData:(DataInfo *)data yPoint:(float) y{
    ...
    float sImgW = WIDTH-2;//缩略图宽带
    float sImgH = sImgW*imgH/imgW;//缩略图高度
    self = [super initWithFrame:CGRectMake(0, y, WIDTH, sImgH+2)];
    if (self) {
...
//        [imageBtn setImageFromUrl:YES withUrl:data.url];//设置图片地质
        ...
        [imageBtn setImageWithURL:url forState:UIControlStateNormal placeholderImage:nil];
